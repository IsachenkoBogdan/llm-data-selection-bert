...
def _plot_label_distribution(df_train: pd.DataFrame, df_val: pd.DataFrame, out_dir: Path):
    ...
    for df, name, offset, color in [
        (df_train, "train", -0.18, "tab:blue"),
        (df_val, "validation", 0.18, "tab:orange"),
    ]:
        vc = df["label"].value_counts(normalize=True).sort_index()
        xs = np.array(list(vc.index), dtype=float) + offset
        ax.bar(xs, vc.values, width=0.35, label=name, alpha=0.85, color=color)

    ax.set_xticks([0, 1])
    ax.set_xticklabels(["negative (0)", "positive (1)"])
    ax.set_ylabel("Fraction")
    ax.set_title("SST-2: label distribution (train vs validation)")
    ax.legend()
...
def _plot_length_histograms(df_train: pd.DataFrame, out_dir: Path):
    """Гистограммы длин через np.histogram (+ аккуратные оси и сетка)."""
    ...
    # ----- words -----
    if len(word_len) > 0:
        bins_w = np.linspace(word_len.min(), word_len.max(), 41)  # 40 бинoв → 41 граница
        counts_w, edges_w = np.histogram(word_len, bins=bins_w)
        axes[0].bar(
            edges_w[:-1],
            counts_w,
            width=np.diff(edges_w),
            align="edge",
            alpha=0.8,
        )
    ...
    if len(char_len) > 0:
        ...
        axes[1].bar(
            edges_c[:-1],
            counts_c,
            width=np.diff(edges_c),
            align="edge",
            alpha=0.8,
        )
    axes[1].set_title("Train sentence length (characters)")
    axes[1].set_xlabel("Characters per sentence")
    axes[1].set_ylabel("Number of examples")
    axes[1].grid(axis="y", linestyle="--", alpha=0.5)

    fig.suptitle("SST-2: length distributions", y=1.03, fontsize=13, fontweight="bold")
    fig.tight_layout()
    fig.savefig(out_dir / "length_histograms.png")
    plt.close(fig)


def _plot_length_by_label(df_train: pd.DataFrame, out_dir: Path):
    """Boxplot длины в разрезе классов."""
    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    ...
    axes[0].set_title("Sentence length (words) by label")
    ...
    axes[1].set_title("Sentence length (chars) by label")
    axes[1].set_ylabel("Characters per sentence")
    axes[1].grid(axis="y", linestyle="--", alpha=0.5)

    fig.suptitle("SST-2: length vs label", y=1.03, fontsize=13, fontweight="bold")
    fig.tight_layout()
    fig.savefig(out_dir / "length_by_label.png")
    plt.close(fig)
